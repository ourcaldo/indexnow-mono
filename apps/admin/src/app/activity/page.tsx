...
export default function ActivityPage() {
  ...
  return (
    <div className="bg-white min-h-full">
      ...
      {/* ─── Table ───────────────────────────────────────── */}
      {isLoading ? (
        <div className="px-8 py-5 space-y-3">{Array.from({ length: 10 }).map((_, i) => <div key={i} className="h-12 bg-gray-50 rounded-lg animate-pulse" />)}</div>
      ) : (
        <>
              ...
              <tbody>
                {logs.map((log: any, idx: number) => (
                  <tr key={log.id} onClick={() => setSelectedLogId(log.id)} className="border-b border-gray-50 last:border-0 hover:bg-blue-50/40 cursor-pointer transition-colors">
                    <td className="pl-8 pr-3 py-3.5 text-sm text-gray-700 tabular-nums">{offset + idx + 1}</td>
                    ...
                    <td className="pl-3 pr-8 py-3.5 text-sm text-gray-700 tabular-nums whitespace-nowrap">{format(new Date(log.created_at), 'MMM d, HH:mm')}</td>
                  </tr>
                ))}
