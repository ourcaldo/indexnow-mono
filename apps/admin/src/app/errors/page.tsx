...
export default function ErrorsPage() {
  ...
  return (
    <div className="bg-white min-h-full">
      ...
      {isLoading ? (
        <div className="px-8 py-5 grid grid-cols-4 gap-4">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="rounded-xl border border-gray-200 h-20 animate-pulse" />)}</div>
      ) : (
        <>
          {/* ─── Stat badges ───────────────────────────────── */}
          <div className="px-8 py-5 border-b border-gray-200 grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          {stats?.trend && (
            <div className="flex items-center gap-2 px-8 py-3 border-b border-gray-200">
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-200">
                      <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wider pl-8 pr-3 py-3">Message</th>
                      <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wider px-3 py-3">Type</th>
                      <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wider px-3 py-3">Severity</th>
                      <th className="text-right text-[11px] font-semibold text-gray-400 uppercase tracking-wider pl-3 pr-8 py-3">Count</th>
                    </tr>
                  </thead>
                  <tbody>
                    {stats.topErrors.map((err: any, i: number) => (
                      <tr key={i} className="border-b border-gray-50 last:border-0">
                        <td className="pl-8 pr-3 py-3 text-sm text-gray-900 truncate max-w-[300px]">{err.message}</td>
                        ...
                        <td className="pl-3 pr-8 py-3 text-sm font-semibold text-gray-900 tabular-nums text-right">{err.count}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {stats?.distributions?.byEndpoint && stats.distributions.byEndpoint.length > 0 && (
            <>
              <div className="px-8 py-3 border-b border-gray-100">
                <h3 className="text-sm font-semibold text-gray-900">By Endpoint</h3>
              </div>
          ...
          {criticalErrors.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wider pl-8 pr-3 py-3">Error</th>
                    <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wider px-3 py-3">Type</th>
                    <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wider pl-3 pr-8 py-3">Time</th>
                  </tr>
                </thead>
                <tbody>
                  {criticalErrors.map((err: CriticalError) => (
                    <tr key={err.id} onClick={() => router.push(`/errors/${err.id}`)} className="border-b border-gray-50 last:border-0 hover:bg-red-50/40 cursor-pointer transition-colors">
                      ...
                      <td className="px-3 py-3.5 text-xs text-gray-500">{err.error_type}</td>
                      <td className="pl-3 pr-8 py-3.5 text-xs text-gray-500 tabular-nums whitespace-nowrap">{format(new Date(err.created_at), 'MMM d, HH:mm')}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
