  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        ...
        {isLoading ? (
          <div className="p-5 space-y-3">{Array.from({ length: 8 }).map((_, i) => <div key={i} className="h-14 bg-gray-50 rounded-lg animate-pulse" />)}</div>
        ) : (
          <>
                ...
                <tbody>
                  {orders.map((order: any, index: number) => (
                    <tr key={order.id} onClick={() => router.push(`/orders/${order.id}`)} className="border-b border-gray-50 last:border-0 hover:bg-blue-50/40 cursor-pointer transition-colors">
                      ...
                      <td className="px-5 py-3.5 text-sm text-gray-600">{order.package?.name || '\u2014'}</td>
                      <td className="px-5 py-3.5 text-sm font-semibold text-gray-900 tabular-nums">{order.currency?.toUpperCase()} {Number(order.amount ?? 0).toLocaleString()}</td>
                      <td className="px-5 py-3.5">
