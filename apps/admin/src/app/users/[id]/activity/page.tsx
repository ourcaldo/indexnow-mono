  ...
  return (
    <div className="bg-white min-h-full">
      <div className="px-8 py-5 border-b border-gray-200">
        <button onClick={() => router.push(`/users/${userId}`)} className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-900 transition-colors mb-3">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wider pl-8 pr-5 py-3">Event</th>
                  <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wider px-5 py-3">Details</th>
                  <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wider pl-5 pr-8 py-3">Time</th>
                </tr>
              </thead>
              <tbody>
                {logs.map((log: any) => (
                  <tr key={log.id} className="border-b border-gray-50 last:border-0">
                    ...
                    <td className="pl-5 pr-8 py-3.5 text-xs text-gray-500 tabular-nums whitespace-nowrap">{format(new Date(log.created_at), 'MMM d, HH:mm')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
