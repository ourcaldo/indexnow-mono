...
export function useUserManagement(): UseUserManagementReturn {
  ...
  const handleResetPassword = useCallback(
    async (userId: string) => {
      setConfirmConfig({
        ...
        message:
          "Are you sure you want to reset this user's password? They will receive a password reset email.",
        confirmText: 'Reset',
        onConfirm: async () => {
          try {
            setActionLoading((prev) => ({ ...prev, resetPassword: true }));

            const response = await authenticatedFetch(ADMIN_ENDPOINTS.RESET_USER_PASSWORD(userId), {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
  ...
  const handleResetQuota = useCallback(
    async (userId: string, onSuccess: () => void) => {
      setConfirmConfig({
        ...
        title: 'Reset Quota',
        message:
          "Are you sure you want to reset this user's daily quota? This will reset their usage to 0.",
        onConfirm: async () => {
          try {
            setActionLoading((prev) => ({ ...prev, resetQuota: true }));

            const response = await authenticatedFetch(ADMIN_ENDPOINTS.RESET_USER_QUOTA(userId), {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
      ...
      setConfirmConfig({
        ...
        onConfirm: async () => {
          try {
            ...
            const response = await authenticatedFetch(ADMIN_ENDPOINTS.EXTEND_SUBSCRIPTION(userId), {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
  ...
  const handleSaveEdit = useCallback(
    async (userId: string, editForm: EditForm, onSuccess: () => void) => {
      try {
  ...
  const handlePackageChangeSubmit = useCallback(
    async (userId: string, selectedPackageId: string, onSuccess: () => void) => {
      ...
      try {
        ...
        const response = await authenticatedFetch(ADMIN_ENDPOINTS.CHANGE_PACKAGE(userId), {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (response.ok) {
          ...
          showError(
            'Action Failed',
            errorData.error || 'Failed to change package. Please try again.'
          );
        }
      } catch (error) {
        logger.error(
