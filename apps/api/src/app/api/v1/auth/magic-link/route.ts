 ...
 * Send a magic link (passwordless login) to user.
...
export const POST = publicApiWrapper(async (request: NextRequest) => {
  try {
    ...
    const [emailCheck, ipCheck] = await Promise.all([
      redisRateLimiter.check(`magic_link_email_${normalizedEmail}`, RATE_LIMIT_EMAIL),
      redisRateLimiter.check(`magic_link_ip_${clientIP}`, RATE_LIMIT_IP),
    ]);
    ...
    const defaultRedirectTo = `${AppConfig.app.dashboardUrl || AppConfig.app.baseUrl}/auth/callback?next=/`;
    ...
    if (otpError) {
      ...
      logger.warn(
        { error: otpError, email: normalizedEmail },
        'Magic link send failed (may be non-existent email)'
      );
    }
    ...
    try {
      await ActivityLogger.logActivity({
        ...
        actionDescription: `Magic link requested for ${normalizedEmail}`,
        targetType: 'user',
        request,
      });
    } catch {
    ...
    return formatSuccess({ message: 'If the email exists, a magic link has been sent.' });
  } catch (error) {
