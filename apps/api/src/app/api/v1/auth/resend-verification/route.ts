...
export const POST = publicApiWrapper(async (request: NextRequest) => {
  try {
    ...
    const clientIP = getClientIP(request) || request.headers.get('cf-connecting-ip') || '127.0.0.1';
    ...
    if (resendError) {
      if (
        ...
        resendError.message?.includes('too_many_requests')
      ) {
        const rateLimitError = await ErrorHandlingService.createError(
          ErrorType.RATE_LIMIT,
      ...
      logger.warn({
        message: 'Resend verification error (hidden from user)',
