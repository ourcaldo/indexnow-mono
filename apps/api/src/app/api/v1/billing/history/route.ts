...
function mapLegacyTransaction(tx: TransactionWithJoins): NormalizedBillingTransaction {
  ...
  return {
    ...
    billing_period:
      tx.metadata && typeof tx.metadata.billing_period === 'string'
...
export const GET = authenticatedApiWrapper(async (request, auth) => {
  ...
  try {
    ...
    const [legacyResult, paddleResult] = await Promise.all([
      ...
      SecureServiceRoleWrapper.executeWithUserSession<{
        ...
        async (db) => {
          ...
          if (
            statusFilter &&
            ['pending', 'proof_uploaded', 'completed', 'failed', 'cancelled', 'refunded'].includes(
              statusFilter
            )
          ) {
    ...
    const allTransactions = [...normalizedLegacy, ...normalizedPaddle].sort(
    ...
    const [completedCount, pendingCount, failedCount, totalAmountResult] = await Promise.all([
      SecureServiceRoleWrapper.executeWithUserSession<number>(
        ...
        {
          userId: auth.userId,
          operation: 'billing_summary_completed',
          source: 'billing/history',
          reason: 'Count completed',
        },
        { table: 'indb_payment_transactions', operationType: 'select' },
        async (db) => {
      ...
      SecureServiceRoleWrapper.executeWithUserSession<number>(
        ...
        {
          userId: auth.userId,
          operation: 'billing_summary_pending',
          source: 'billing/history',
          reason: 'Count pending',
        },
        { table: 'indb_payment_transactions', operationType: 'select' },
        async (db) => {
          const { count } = await db
      ...
      SecureServiceRoleWrapper.executeWithUserSession<number>(
        ...
        {
          userId: auth.userId,
          operation: 'billing_summary_failed',
          source: 'billing/history',
          reason: 'Count failed',
        },
        { table: 'indb_payment_transactions', operationType: 'select' },
      ...
      SecureServiceRoleWrapper.executeWithUserSession<number>(
        ...
        {
          userId: auth.userId,
          operation: 'billing_summary_amount',
          source: 'billing/history',
          reason: 'Sum completed amount',
        },
        { table: 'indb_payment_transactions', operationType: 'select' },
        async () => {
    ...
    const paddleCompletedCount = normalizedPaddle.filter(
