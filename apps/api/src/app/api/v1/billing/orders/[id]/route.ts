...
import {
  DbTransactionRow as TransactionRow,
...
export const GET = authenticatedApiWrapper(async (request: NextRequest, auth, context) => {
  const params = (await context.params) as Record<string, string>;
  const id = params?.id;

  if (!id) {
    return formatError(
      await createStandardError(ErrorType.VALIDATION, 'Order ID is required', {
        statusCode: 400,
  ...
  if (!transaction) {
    return formatError(
      await createStandardError(ErrorType.NOT_FOUND, 'Order not found or access denied', {
        statusCode: 404,
        severity: ErrorSeverity.LOW,
        metadata: { orderId: id },
      })
  ...
  const orderData = {
    ...
    transaction_id: (transaction as Record<string, unknown>).gateway_transaction_id as
      | string
      | undefined,
