...
export const POST = authenticatedApiWrapper(async (request, auth) => {
    try {
        ...
        const transaction = await SecureServiceRoleWrapper.executeWithUserSession<PaymentTransactionRow | null>(
            auth.supabase,
            {
                userId: auth.userId,
        ...
        const structuredError = await ErrorHandlingService.createError(
            ...
            { severity: ErrorSeverity.HIGH, userId: auth.userId, endpoint: '/api/v1/billing/upload-proof', method: 'POST', statusCode: 500 }
        );
