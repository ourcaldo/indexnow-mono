...
async function getWebhookSecretFromDatabase(): Promise<string> {
  ...
  if (!webhookSecret) {
    throw new Error(
      'PADDLE webhook secret not found in database. Please update indb_payment_gateways.api_credentials with actual webhook_secret.'
    );
  }
...
export const POST = publicApiWrapper(async (request: NextRequest) => {
  const rateLimited = await checkRouteRateLimit(
    request,
    { maxAttempts: 50, windowMs: 60_000 },
    'webhook'
  );
    ...
    const upsertResult = await SecureServiceRoleWrapper.executeSecureOperation<{
      ...
      async () => {
        ...
        if (!existing) {
          ...
          const { error } = await supabaseAdmin.from('indb_paddle_webhook_events').upsert(
            {
              event_id,
              event_type,
...
async function verifyPaddleSignature(
  ...
  try {
    ...
    if (
      timestampPart.length !== 2 ||
      timestampPart[0] !== 'ts' ||
      signaturePart.length !== 2 ||
      signaturePart[0] !== 'h1'
    ) {
    ...
    if (timeDiffMs > WEBHOOK_TIMESTAMP_TOLERANCE_MS) {
      return {
        valid: false,
        error: 'Timestamp outside tolerance window (replay attack protection)',
      };
    }
