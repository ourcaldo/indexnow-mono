...
import { KeywordBankEntity, KeywordBankQueryResult, CacheStats } from '../types/KeywordBankTypes';
...
const DEFAULT_CONFIG: SeRankingServiceConfig = {
  apiUrl: 'https://api.seranking.com', // Base URL only — ApiRequestBuilder appends /v1/keywords/export
...
export class SeRankingService extends EventEmitter implements ISeRankingService {
  ...
  private async performInitialHealthCheck(): Promise<void> {
    try {
      ...
      if (healthResult.status !== 'healthy') {
        this.log(
          'warn',
          `Initial health check shows ${healthResult.status} status: ${healthResult.error_message}`
        );
      }
    } catch (error) {
  ...
  async getStatus(): Promise<SystemStatus> {
    try {
      ...
      const queueStatus = await this.enrichmentQueue
        ?.getQueueStats()
        .catch(() => ({ processing: 0 }));
      ...
      return {
        ...
        quotaStatus:
          quotaStatus && 'data' in quotaStatus && quotaStatus.success
            ? quotaStatus.data!
            : {
                current_usage: 0,
                quota_limit: 0,
        ...
        activeJobs:
          queueStatus && 'processingJobs' in queueStatus
            ? queueStatus.processingJobs
            : (queueStatus as { processing?: number })?.processing || 0,
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          cacheStats,
          queueStats,
          quotaStatus: quotaResult.success
            ? quotaResult.data!
            : {
                current_usage: 0,
                quota_limit: 0,
