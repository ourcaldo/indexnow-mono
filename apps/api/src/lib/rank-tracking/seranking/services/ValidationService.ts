...
import { SeRankingApiError } from '../client/SeRankingApiClient';
...
export class ValidationService {
   ...
   */
  static validateKeywordExportRequest(
    request: unknown
  ): ValidationResult<SeRankingKeywordExportRequest> {
    try {
  ...
  static validateKeywordBankUpdate(data: unknown): ValidationResult<KeywordBankUpdate> {
    try {
      ...
      return {
        isValid: true,
        data: validatedData as KeywordBankUpdate,
        errors: [],
        warnings,
      };
    } catch (error) {
  ...
  static validateKeywordBankQuery(query: unknown): ValidationResult<KeywordBankQuery> {
    try {
      ...
      if (
        validatedQuery.min_volume &&
        validatedQuery.max_volume &&
      ...
      if (
        validatedQuery.min_difficulty &&
        validatedQuery.max_difficulty &&
      ...
      return {
        isValid: true,
        data: validatedQuery as KeywordBankQuery,
        errors: [],
        warnings,
      };
    } catch (error) {
  ...
  static createValidationError(
    ...
    return new SeRankingApiError(message, SeRankingErrorType.INVALID_REQUEST_ERROR, {
      retryable: false,
