...
type RankKeywordsRow = Database['public']['Tables']['indb_rank_keywords']['Row'];
...
export class RankTrackingService {
  ...
  async getUserKeywords(
    ...
    const filters: Record<string, Json> = { user_id: userId };
  ...
  async updateKeyword(
    ...
    const { data, error } = await this.supabaseService.update('indb_rank_keywords', updateData as Record<string, Json>, { id: keywordId, user_id: userId });
  ...
  async getRankHistory(
    ...
    const { data, error } = await this.supabaseService.query('indb_keyword_rank_history', {
  ...
  async getUserDomains(userId: string): Promise<RankTrackingDomain[]> {
    ...
    const domainCounts = (keywords as { domain: string }[]).reduce((acc: Record<string, number>, keyword) => {
  ...
  async getKeywordUsage(userId: string): Promise<KeywordUsage> {
    const { data, count } = await this.supabaseService.query<{ id: string; is_active: boolean }>('indb_rank_keywords', {
      filters: { user_id: userId },
      select: 'id, is_active',
    });
    ...
    const { data: userProfile } = await this.supabaseService.query<{ keyword_limit: number }>('indb_auth_user_profiles', {
      filters: { user_id: userId },
  ...
  private async saveRankHistory(
    ...
    await this.supabaseService.insert('indb_keyword_rank_history', {
   ...
   */
  private mapDatabaseKeywordToModel(data: RankKeywordsRow): RankKeyword {
    return {
      id: data.id,
      userId: data.user_id,
      ...
      device: data.device as RankKeyword['device'],
      searchEngine: data.search_engine as RankKeyword['searchEngine'],
