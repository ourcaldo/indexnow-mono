...
export interface SecurityMiddlewareResult {
  ...
  responseProcessor?: (data: Json) => Promise<NextResponse>;
...
export class UnifiedSecurityMiddleware {
  ...
  async processRequest(
    ...
    try {
      ...
      let responseProcessor: ((data: Json) => Promise<NextResponse>) | undefined;
      if (effectiveConfig.enableResponseEncryption) {
        responseProcessor = async (responseData: Json) => {
  ...
  private applySecurityLevelDefaults(config: UnifiedSecurityConfig): UnifiedSecurityConfig {
    ...
    const levelDefaults: Record<'basic' | 'enhanced' | 'maximum', UnifiedSecurityConfig> = {
      basic: {
