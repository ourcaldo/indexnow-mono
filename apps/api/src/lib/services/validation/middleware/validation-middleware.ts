...
import { VALIDATION_PATTERNS, BaseSchemas, type Json } from '@indexnow/shared';
...
export interface ValidationOptions {
  ...
  validateParams?: z.ZodSchema<unknown>;
...
export class ValidationMiddleware {
  ...
  async validateRequest(
    ...
    const sanitizedData: Record<string, Json> = {};
  ...
  ): Promise<{ isValid: boolean; errors: string[]; warnings: string[]; data?: Json }> {
    const errors: string[] = [];
    const warnings: string[] = [];
  ...
  private validateQueryParams(
    request: NextRequest,
    schema: z.ZodSchema<unknown>
  ): { isValid: boolean; errors: string[]; warnings: string[]; data?: Json } {
    const errors: string[] = [];
    const warnings: string[] = [];
   ...
   */
  private async validateAuthentication(request: NextRequest): Promise<{ isValid: boolean; errors: string[]; user?: Json }> {
    try {
   ...
   */
  private sanitizeHtmlContent(data: Json): Json {
   ...
   */
  private sanitizeUrlContent(data: Json): { data: Json; warnings: string[] } {
    const warnings: string[] = [];

    const sanitizeValue = (value: Json): Json => {
