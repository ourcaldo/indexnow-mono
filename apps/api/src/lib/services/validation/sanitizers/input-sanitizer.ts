...
import { VALIDATION_PATTERNS, FIELD_LIMITS, type Json } from '@indexnow/shared';
...
export class InputSanitizer {
   ...
   */
  private sanitizeJsonObject(obj: Json): Json {
    ...
    if (obj !== null && typeof obj === 'object') {
      ...
      for (const [key, value] of Object.entries(obj)) {
        result[key] = this.sanitizeJsonObject(value);
      }
      return result;
    }
   ...
   */
  sanitizeInput(input: Json, type: 'text' | 'email' | 'url' | 'phone' | 'json' | 'filename' | 'search'): string {
    if (!input) {
      return '';
    }
