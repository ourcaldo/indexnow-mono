...
export default function BillingPage() {
  ...
  const handleSubscribe = async (packageId: string, period: string) => {
    try {
      ...
      handleApiError(error, {
        context: 'BillingPage.handleSubscribe',
  ...
  if (error) {
    return (
      <div className="py-12 text-center">
        <ErrorState title="Error Loading Billing Data" message={error} onRetry={loadAllData} />
      </div>
    );
  }

  const currentPlan = packagesData?.packages.find(
    (pkg) => pkg.id === packagesData.current_package_id
  );
  const currentUserBillingPeriod = billingData?.currentSubscription?.billing_period || 'monthly';
  const currentPlanPricing = currentPlan
    ? getBillingPeriodPrice(currentPlan, currentUserBillingPeriod)
    : null;
    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="mb-6 flex items-center justify-between">
          <h2 className="text-foreground text-2xl font-bold" data-testid="text-heading-plans">
            Available Plans
          </h2>
          <div className="bg-success/10 text-success rounded-full px-3 py-1 text-xs font-medium">
        ...
        <PricingCards
          ...
          setSelectedBillingPeriod={(period: string) =>
            setSelectedBillingPeriod(period === 'annual' ? 'yearly' : 'monthly')
          }
      ...
      {subscriptionData?.hasSubscription && subscriptionData.subscription && (
        <div
          className="rounded-xl bg-white shadow-sm ring-1 ring-red-900/10"
        ...
        >
          ...
          <div className="flex items-center justify-between border-t border-gray-200 px-6 py-6">
            <div>
              <h3 className="font-medium text-gray-900">Cancel Subscription</h3>
              <p className="text-sm text-gray-600">
                All services will be stopped at the end of your billing cycle.
              </p>
            </div>
            <button
              onClick={() => setShowCancelDialog(true)}
              className="rounded-md bg-red-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-red-700 focus:ring-2 focus:ring-red-500 focus:ring-offset-2 focus:outline-none"
      ...
      {showCancelDialog &&
        ...
        subscriptionData.subscription?.paddle_subscription_id && (
          <CancelSubscriptionDialog
