...
var optionalUrl = z3.preprocess(
  (val) => typeof val === "string" && val.trim() === "" ? void 0 : val,
  z3.string().url().optional()
);
var AppSchema = z3.object({
  name: z3.string().default("IndexNow Studio"),
  version: z3.string().default("1.0.0"),
  environment: z3.enum(["development", "staging", "production"]).default("production"),
  baseUrl: z3.string().url().default("http://localhost:3000"),
  dashboardUrl: optionalUrl,
  backendUrl: optionalUrl,
  apiBaseUrl: optionalUrl,
  port: z3.coerce.number().default(3e3),
  allowedOrigins: z3.array(z3.string()).default([])
});
var SupabaseSchema = z3.object({
  url: z3.string().url(),
  anonKey: z3.string().min(1),
  serviceRoleKey: z3.string().min(1).optional(),
  jwtSecret: z3.string().min(1).optional(),
  bucketName: z3.string().default("indexnow-public")
});
var SecuritySchema = z3.object({
  encryptionKey: z3.string().min(1).optional(),
  encryptionMasterKey: z3.string().min(1).optional(),
  jwtSecret: z3.string().min(1).optional(),
  systemApiKey: z3.string().min(1).optional()
});
var RedisSchema = z3.object({
...
var BullMQSchema = z3.object({
  concurrency: z3.object({
    rankCheck: z3.coerce.number().default(5),
    email: z3.coerce.number().default(10),
    payments: z3.coerce.number().default(3)
  }),
  rateLimit: z3.object({
    rankCheck: z3.object({
      max: z3.coerce.number().default(28),
      duration: z3.coerce.number().default(6e4)
    }),
    email: z3.object({
      max: z3.coerce.number().default(50),
      duration: z3.coerce.number().default(6e4)
    })
  })
});
var PaddleSchema = z3.object({
  apiKey: z3.string().min(1).optional(),
  webhookSecret: z3.string().min(1).optional(),
  clientToken: z3.string().min(1).optional(),
  environment: z3.enum(["sandbox", "production"]).default("sandbox")
});
var MonitoringSchema = z3.object({
  sentry: z3.object({
    dsn: optionalUrl,
    environment: z3.string().default("development"),
    traceSampleRate: z3.coerce.number().default(0.1),
    replaysSessionRate: z3.coerce.number().default(0.1),
    replaysErrorRate: z3.coerce.number().default(1)
  }),
  posthog: z3.object({
    key: z3.string().optional(),
    host: z3.string().url().default("https://app.posthog.com")
  }),
  ga4: z3.object({
    measurementId: z3.string().optional()
  }),
...
var EmailSchema = z3.object({
  smtp: z3.object({
    ...
    port: z3.coerce.number().optional(),
    user: z3.string().optional(),
    pass: z3.string().optional(),
    fromName: z3.string().default("IndexNow"),
...
var createApiKeySchema = z4.object({
  name: z4.string().min(1, "API key name is required").max(100, "Name must be less than 100 characters"),
  description: z4.string().max(500, "Description must be less than 500 characters").optional(),
  scopes: z4.array(z4.string()).min(1, "At least one scope is required"),
  expiresAt: z4.date().optional()
});

// src/types/api/requests/PaymentRequests.ts
import { z as z5 } from "zod";
var customerInfoSchema2 = z5.object({
  firstName: z5.string().min(1, "First name is required").max(50, "First name must be less than 50 characters"),
  lastName: z5.string().min(1, "Last name is required").max(50, "Last name must be less than 50 characters"),
  email: z5.string().email("Please enter a valid email address"),
  phone: z5.string().min(1, "Phone number is required"),
  address: z5.object({
    street: z5.string().min(1, "Street address is required"),
  ...
  company: z5.object({
    name: z5.string(),
    taxId: z5.string().optional(),
    industry: z5.string().optional()
  }).optional()
});
var createPaymentSchema = z5.object({
  packageId: z5.string().uuid("Invalid package ID"),
  billingPeriod: z5.enum(["monthly", "quarterly", "biannual", "annual"]),
  paymentMethod: z5.enum(["paddle", "credit-card"]),
  customerInfo: customerInfoSchema2,
  promoCode: z5.string().optional(),
  isTrialToSubscription: z5.boolean().optional(),
  returnUrl: z5.string().url().optional(),
...
var createSubscriptionSchema = z5.object({
  ...
  billingPeriod: z5.enum(["monthly", "quarterly", "biannual", "annual"]),
  customerInfo: customerInfoSchema2,
  paymentMethod: z5.string().min(1, "Payment method is required"),
  tokenId: z5.string().optional(),
  startDate: z5.date().optional(),
  trialDays: z5.number().min(0).max(365).optional(),
  promoCode: z5.string().optional()
});
var createRefundSchema = z5.object({
  transactionId: z5.string().uuid("Invalid transaction ID"),
  amount: z5.number().positive().optional(),
  reason: z5.enum(["duplicate", "fraud", "requested_by_customer", "other"]),
  reasonDetails: z5.string().max(500, "Reason details must be less than 500 characters").optional(),
  notifyCustomer: z5.boolean().optional()
});
var validatePromoCodeSchema = z5.object({
  code: z5.string().min(1, "Promo code is required"),
  packageId: z5.string().uuid("Invalid package ID"),
  billingPeriod: z5.enum(["monthly", "quarterly", "biannual", "annual"]),
  userId: z5.string().uuid().optional()
});

// src/types/queues/QueueTypes.ts
import { z as z6 } from "zod";
var ImmediateRankCheckJobSchema = z6.object({
  keywordId: z6.string().uuid(),
  userId: z6.string().uuid(),
  domainId: z6.string().uuid(),
  keyword: z6.string(),
  countryCode: z6.string(),
  device: z6.enum(["desktop", "mobile", "tablet"])
});
var EmailJobSchema = z6.object({
  to: z6.string().email(),
  subject: z6.string(),
  template: z6.enum([
  ...
  data: z6.record(z6.any())
});
var PaymentWebhookJobSchema = z6.object({
  orderId: z6.string(),
  transactionId: z6.string(),
  status: z6.enum(["pending", "settlement", "expire", "cancel", "deny"]),
  paymentType: z6.string(),
  webhookData: z6.record(z6.any())
});
var AutoCancelJobSchema = z6.object({
  scheduledAt: z6.string().datetime()
});
var KeywordEnrichmentJobSchema = z6.object({
  scheduledAt: z6.string().datetime()
});
...
export {
  ...
  customerInfoSchema2 as customerInfoSchema,
  ensureProtocol,
  escapeLikePattern,
  extractDomain,
  findCountryByCode,
