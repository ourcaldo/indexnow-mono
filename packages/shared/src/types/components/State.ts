...
import { NotificationType as AppNotificationType } from '../../constants/AppConstants';
...
export interface NotificationActions {
  ...
  clearNotificationsByType: (type: AppNotificationType) => void;
}

// Form state types
export interface FormState<T = Record<string, Json>> {
  data: T;
...
export interface FormActions<T = Record<string, Json>> {
  setFieldValue: (field: keyof T, value: Json) => void;
...
// Table state types
export interface TableState<T = Record<string, Json>> {
  data: T[];
  filteredData: T[];
  selectedRows: T[];
  currentPage: number;
...
export interface TableActions<T = Record<string, Json>> {
  setData: (data: T[]) => void;
  ...
  setFilter: (key: string, value: Json) => void;
  clearFilters: () => void;
  setLoading: (isLoading: boolean) => void;
}
...
export interface ValidationActions {
  validateField: (field: string, value: Json) => Promise<void>;
...
export interface WizardStep {
  ...
  isValid: boolean;
  isOptional: boolean;
  component?: React.ComponentType<Record<string, Json>>;
}
...
export interface WizardActions {
  ...
  setStepData: (stepId: string, data: Json) => void;
...
export interface DashboardState {
  ...
  recentActivity: Record<string, Json>[];
  charts: {
    jobsOverTime: Record<string, Json>[];
    urlsOverTime: Record<string, Json>[];
    successRate: Record<string, Json>[];
    quotaUsage: Record<string, Json>[];
  };
  isLoading: boolean;
  lastUpdated?: Date;
}
