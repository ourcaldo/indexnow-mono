...
export interface PaginatedResponse<T = Json> extends ApiResponse<T[]> {
  pagination: {
    page: number;
    limit: number;
    total: number;
...
export interface CacheableResponse<T = Json> extends ApiResponse<T> {
  cacheInfo?: {
    cached: boolean;
    key: string;
    ttl: number;
...
// Webhook types
export interface WebhookPayload<T = Json> {
  event: string;
  data: T;
  timestamp: string;
  signature?: string;
...
export interface BulkOperation<T = Json> {
...
export interface BulkOperationResult<T = Json> {
  successful: T[];
  failed: Array<{
    item: T;
    error: string;
