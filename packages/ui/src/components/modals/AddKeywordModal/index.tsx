  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent
        ...
        data-testid="modal-add-keyword"
      >
        {/* Header - Compact */}
        <DialogHeader className="border-border border-b px-6 pt-5 pb-3 sm:px-5">
          <DialogTitle className="text-lg sm:text-xl">Add Keywords to Track</DialogTitle>
        ...
        <div className="bg-secondary/30 flex items-center justify-center gap-2 px-6 py-3 sm:px-5">
          <div
            className={`flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs sm:text-sm ${step >= 1 ? 'bg-primary text-primary-foreground font-medium' : 'bg-background text-muted-foreground border-border border'}`}
          >
            <span
              className={`flex h-5 w-5 items-center justify-center rounded-full text-xs ${step >= 1 ? 'bg-primary-foreground text-primary' : 'bg-secondary'}`}
            >
              1
            </span>
          ...
          <div className={`h-px w-6 sm:w-8 ${step >= 2 ? 'bg-primary' : 'bg-border'}`}></div>
          <div
            className={`flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs sm:text-sm ${step >= 2 ? 'bg-primary text-primary-foreground font-medium' : 'bg-background text-muted-foreground border-border border'}`}
          >
            <span
              className={`flex h-5 w-5 items-center justify-center rounded-full text-xs ${step >= 2 ? 'bg-primary-foreground text-primary' : 'bg-secondary'}`}
            >
              2
            </span>
        ...
        <div className="max-h-[calc(90vh-180px)] overflow-y-auto px-6 py-4 sm:px-5">
            ...
            <ErrorState
              ...
              message={
                domainsError instanceof Error
                  ...
                  : 'An error occurred while loading domains'
              }
            ...
            <>
              ...
              {step === 2 && (
                <div className="space-y-4">
                  ...
                  <div className="border-border -mx-1 flex gap-2 border-t pt-3">
                    <Button
