...
export function usePaymentHistory(): UsePaymentHistoryReturn {
  ...
  const fetchTransactions = useCallback(async (page = 1, limit = 10, filters?: PaymentFilters) => {
    try {
      ...
      const response = await authenticatedFetch(buildEndpoint(BILLING_ENDPOINTS.TRANSACTIONS, Object.fromEntries(queryParams)))
  ...
  const fetchInvoices = useCallback(async (page = 1, limit = 10) => {
    try {
      const response = await authenticatedFetch(buildEndpoint(`${BILLING_ENDPOINTS.TRANSACTIONS.replace('/transactions', '/invoices')}`, {page: page.toString(), limit: limit.toString()}))
  ...
  const fetchPaymentStats = useCallback(async (timeRange: 'all' | '1y' | '6m' | '3m' | '1m' = 'all') => {
    try {
      const response = await authenticatedFetch(buildEndpoint(`${BILLING_ENDPOINTS.TRANSACTIONS.replace('/transactions', '/stats')}`, {timeRange}))
  ...
  const retryPayment = useCallback(async (transactionId: string) => {
    try {
      const response = await authenticatedFetch(`${BILLING_ENDPOINTS.TRANSACTION_BY_ID(transactionId)}/retry`, {
        method: 'POST'
      })
  ...
  const downloadInvoice = useCallback(async (invoiceId: string) => {
    try {
      const response = await authenticatedFetch(`${BILLING_ENDPOINTS.TRANSACTIONS.replace('/transactions', '/invoices')}/${invoiceId}/download`)
